...
import { UrgencyModal, type UrgentItem } from "@/components/UrgencyModal";
...
export default async function Dashboard() {
  ...
  // Assigned training stays on the dashboard for 6 months after it's assigned
  // (whether or not it's been completed), then drops off. Anchored to the
  // assignment date, not completion. Grading (KRA) reads assignments separately,
  // so a course leaving the dashboard never changes anyone's score.
  const VISIBILITY_MONTHS = 6;
  const assignedSince = new Date();
  assignedSince.setMonth(assignedSince.getMonth() - VISIBILITY_MONTHS);

  const [myAssignments, modules, statuses, achievements, leaderboard, trajectory] =
    await Promise.all([
      prisma.assignment.findMany({
        where: { userId, createdAt: { gte: assignedSince } },
        include: {
          video: true,
          module: {
            include: {
        ...
        where:
          role === "ADMIN"
            ...
            : {
                course: { published: true },
                OR: [
                  { assignments: { some: { userId, createdAt: { gte: assignedSince } } } },
                  { videos: { some: { assignments: { some: { userId, createdAt: { gte: assignedSince } } } } } },
                ],
              },
        include: {
          course: true,
  ...
  // Urgency nudge: unfinished, graded assignments (skip auto-generated catch-up
  // material, which isn't an obligation). Shown once per day by UrgencyModal.
  const urgentItems: UrgentItem[] = myAssignments
    .filter((a) => a.status === "PENDING" && !a.autoGenerated)
    .map((a) => {
      const targetVideoId =
        a.kind === "VIDEO"
          ? a.videoId
          : a.kind === "MODULE" && a.module
            ? (a.module.videos.find((v) => !v.progresses[0]?.completed)?.id ??
              a.module.videos[0]?.id ??
              null)
            : null;
      return { title: a.title, targetVideoId, dueAt: a.dueAt?.toISOString() ?? null };
    });
  ...
  return (
    <div className="min-h-screen">
      ...
      <UrgencyModal items={urgentItems} />
    </div>
  );
}
