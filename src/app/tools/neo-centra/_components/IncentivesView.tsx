        ...
        <>
          ...
          {me && (() => {
            ...
            const cards = [
              ...
              { ...BUCKETS[1], value: inr(b.billing.billedInPeriod), suffix: b.billing.billedHours > 0 ? `${b.billing.billedHours}h` : undefined, sub: `${b.billing.taskCount} task${b.billing.taskCount === 1 ? "" : "s"}`, negative: false, ...leaderPct(b.billing.billedInPeriod, maxes.b2) },
  ...
  return (
    <>
      <tr className={`border-b border-border/60 ${canDrill ? "cursor-pointer hover:bg-muted/40" : ""} transition`} onClick={canDrill ? onToggle : undefined}>
        ...
        <td className="px-3 py-3 text-right"><div className="font-semibold">{inr(b.billing.billedInPeriod)}</div><div className="text-[10.5px] text-ink-faint">{b.billing.taskCount} tasks{b.billing.billedHours > 0 ? ` · ${b.billing.billedHours}h` : ""}</div></td>
